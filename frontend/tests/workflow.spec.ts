...
for (const imageFile of [
  ...
  test(`upload ${imageFile}, original-coordinate ROI, three upstream results including full image, ground truth, saved detail and dashboards`, async ({
    ...
    const configs = await (
      await request.get(`${backend}/api/pipelines`)
    ).json();
    for (const config of configs) {
      expect(
        (
          await request.put(`${backend}/api/pipelines/${config.pipeline_id}`, {
            data: { enabled: true },
          })
        ).ok(),
      ).toBeTruthy();
    ...
    await page
      ...
      .first()
      .setInputFiles(path.resolve(imageFile));
    const viewer = page.getByTestId("document-viewer");
    await expect(viewer).toBeVisible();
    await expect(
      viewer.getByText(t("Loading document…"), { exact: true }),
    ).toBeHidden();
    await page
      .getByRole("button", { name: t("Draw test region"), exact: true })
      .click();
    ...
    const origin = {
      x: box.x + (box.width - 1000 * scale) / 2,
      y: box.y + (box.height - 1320 * scale) / 2,
    };
    ...
    await page.mouse.move(origin.x + 80 * scale, origin.y + 225 * scale, {
      steps: 12,
    });
    await page.mouse.up();
    await expect(
    ...
    expect(
      await viewer.getByText(/^พื้นที่ที่เลือก \(ROI\) \(/).textContent(),
    ).toEqual(roiBeforeZoom);
    await page
    ...
    const responsePromise = page.waitForResponse(
      (response) =>
        /\/api\/test-cases\/[^/]+\/run$/.test(response.url()) &&
    ...
    const hashes = result.runs
      .slice(0, 2)
      .map((run: { crop_sha256: string }) => run.crop_sha256);
    ...
    await page
      ...
      .click();
    await expect(
      page.getByText(t("SAME INPUT"), { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByAltText(t("Selected test region crop")),
    ).toBeVisible();
    const mintCard = page.locator(
      'article[aria-label="ผลลัพธ์ Mint Pipeline"]',
    );
    await expect(page.locator("main")).not.toContainText(/MOCK|โหมดจำลอง/);
    await mintCard.getByRole("button", { name: /^กรอบที่ 1/ }).click();
    await expect(
      mintCard.getByRole("button", { name: /^กรอบที่ 1/ }),
    ...
    expect(await page.getByTestId("result-text-mint").textContent()).toBe(
      prediction,
    );
    ...
    const diagnostic = page.getByRole("region", {
      name: t("Crop and Gateway debug information"),
    });
    for (const label of [
      "ภาพต้นฉบับ / ภาพอินพุต",
      t("Encoded input"),
      t("Detector"),
      t("Recognizer"),
      t("Total / Gateway"),
    ...
    for (const run of persisted.runs) {
      ...
      await expect(
        card.getByText(t("CONFIDENCE"), { exact: true }),
      ).toBeVisible();
    }
    ...
    await page.route(detailEndpoint, (route) =>
      route.fulfill({
        json: {
          ...persisted,
          runs: persisted.runs.map((run, index) =>
            index ? run : { ...run, crop_sha256: "0".repeat(64) },
          ),
        },
      }),
    );
    ...
    await page
      ...
      .click();
    await expect(
      page.getByText(t("DIFFERENT INPUT"), { exact: true }),
    ).toBeVisible();
    await page.unroute(detailEndpoint);
    await page.goto("/history");
    await page
      .locator(`a[href="/test/${result.test_case_id}"]`)
      .first()
      .click();
    await page
      .getByTestId("technical-details")
    ...
    fs.writeFileSync(
      "test-results/final-benchmark.json",
      JSON.stringify(
        {
          test_case_id: persisted.id,
          document_id: persisted.document_id,
          runs: persisted.runs.map((run: PipelineRun) => ({
            pipeline_id: run.pipeline_id,
            crop_sha256: run.crop_sha256,
            crop_width: run.crop_width,
            crop_height: run.crop_height,
    ...
    for (const route of [
      ...
      await expect(
        page.getByText("Cannot reach the backend", { exact: false }),
      ).toHaveCount(0);
    }
...
test("ROI move and resize, selectable Auto ROI, and failure recovery", async ({
  ...
  const saved = await (
    await request.post(`${backend}/api/test-cases`, {
      data: {
        ...
        roi: { x1: 100, y1: 200, x2: 700, y2: 400 },
      },
    })
  ).json();
  ...
  const point = async (x: number, y: number) => {
    ...
    const scale = Math.min(
      (frame.width - 64) / 1000,
      (frame.height - 64) / 1320,
      1,
    );
    return {
      x: frame.x + (frame.width - 1000 * scale) / 2 + x * scale,
      y: frame.y + (frame.height - 1320 * scale) / 2 + y * scale,
    };
  };
  const readROI = async () =>
    (await viewer.getByText(/^พื้นที่ที่เลือก \(ROI\) \(/).textContent())!
  ...
  await page
    .getByRole("button", { name: t("Auto Detect"), exact: true })
    .click();
  await expect(page.locator("main").getByRole("alert")).toContainText(
    "ไม่สามารถตรวจหาพื้นที่อัตโนมัติได้ในขณะนี้",
  );
  await expect(
    page.getByRole("button", { name: t("Run all pipelines"), exact: true }),
  ).toBeEnabled();
  ...
  await expect(
    page
      .locator('article[aria-label="ผลลัพธ์ Mint Pipeline"]')
      .getByRole("button", { name: /^กรอบที่ 1/ }),
  ).toHaveAttribute("aria-pressed", "true");
});
...
test("settings save and upstream connection test", async ({ page }) => {
  ...
  await expect(
    card.getByRole("button", { name: t("Test connection"), exact: true }),
  ).toBeDisabled();
  await card
    .getByRole("button", { name: t("Save changes"), exact: true })
    .click();
  ...
  await card
    ...
    .click();
  await expect(
    card.getByRole("button", { name: t("Saved"), exact: true }),
  ).toBeVisible();
  await card
    .getByRole("button", { name: t("Test connection"), exact: true })
    .click();
  await expect(
    card.getByText("พร้อมใช้งาน", { exact: true }).first(),
  ).toBeVisible();
});
