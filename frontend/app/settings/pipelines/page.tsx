...
function PipelineCard({ pipeline }: { pipeline: PipelineConfig }) {
  ...
  const status = !saved.enabled
    ...
    : notConfigured
      ? t("Not configured")
      : (saved.last_connection_status
          ? statusLabel(saved.last_connection_status)
          : null) || t("Availability not checked");
  const badgeStyle = !saved.enabled
    ? "bg-slate-100 text-slate-600"
    : notConfigured
      ? "bg-amber-50 text-amber-700"
      : saved.last_connection_status === "available"
        ? "bg-teal-50 text-teal-700"
        : saved.last_connection_status === "unavailable"
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const updated = await updatePipeline(saved.pipeline_id, {
        ...form,
        name: form.name.trim(),
        base_url: form.base_url.trim(),
        endpoint: form.endpoint.trim(),
        file_field_name: form.file_field_name.trim(),
        engine: form.engine.trim() || null,
      });
      setSaved(updated);
      setForm(editable(updated));
      setMessage(t("Pipeline settings saved."));
    } catch (cause) {
      setError(
        cause instanceof Error
          ? userError(cause.message)
          : t("Could not save this pipeline."),
      );
    } finally {
      setBusy(null);
    }
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-base font-semibold text-slate-800">
                {pipelineLabel(saved.pipeline_id, saved.name)}
              </h2>
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold ${badgeStyle}`}
              >
            ...
            <p className="mt-1 max-w-xl text-xs leading-5 text-slate-500">
              {DESCRIPTIONS[saved.pipeline_id] ||
                t("Configurable OCR pipeline adapter.")}
            </p>
          </div>
        </div>
        <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1 font-mono text-[10px] text-slate-400">
          {saved.pipeline_id}
        </span>
      </div>
      <form onSubmit={save}>
        <fieldset
          disabled={busy !== null}
          className="min-w-0 border-0 p-5 sm:p-6"
        >
          <div className="settings-summary">
            ...
            <span className="badge neutral">
              {busy === "test" ? "กำลังตรวจสอบ" : status}
            </span>
          </div>
          <details className="technical-panel">
            <summary>รายละเอียดขั้นสูง</summary>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 mt-4">
              ...
              <label className="field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    change(
                      "request_format",
                      event.target.value as PipelineConfig["request_format"],
                    )
                  }
                >
                  <option value="multipart">{t("Multipart form data")}</option>
                  <option value="json_base64">{t("JSON / Base64")}</option>
                </select>
              </label>
              <label className="field sm:col-span-2">
                ...
                <span className="text-[10px] font-normal text-slate-400">
                  {t("ระบุที่อยู่ Gateway โดยไม่ใส่ข้อมูลลับ")}
                </span>
              </label>
              <label className="field">
                {t("HTTP method")}
                <input className="input" value="POST" readOnly />
              </label>
              <label className="field">
                {t("Endpoint")}
                <input
            ...
            <label className="flex cursor-pointer items-start gap-2.5">
              ...
              <span>
                ...
                <span className="mt-1 block text-[10px] text-slate-500">
                  {t("Available for benchmark runs")}
                </span>
              </span>
            </label>
          ...
          {message && (
            <div
              className="mt-4 flex items-start gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs leading-5 text-slate-600"
              role="status"
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 bg-slate-50/40 px-5 py-4 sm:px-6">
          <div>
            ...
            {dirty && (
              <p className="mt-1 text-[10px] text-amber-700">
                {t("Unsaved changes. Save before testing the connection.")}
              </p>
            )}
          </div>
          <div className="flex gap-2">
            <button
              ...
              title={
                dirty
                  ? t("Save your changes before testing")
                  : t("Test the saved pipeline configuration")
              }
    ...
    <div className="page-stack">
      ...
      {loading ? (
        ...
        <div className="panel empty-state py-16">
          <Cpu size={30} className="mx-auto mb-4 text-indigo-300" />
          <h2 className="font-semibold text-slate-800">
            {t("No pipeline configurations found")}
          </h2>
          <p className="mt-2 text-sm text-slate-500">
            {t(
              "Initialize the backend database to create the three default pipeline adapters.",
            )}
          </p>
          <button
